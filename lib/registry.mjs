...
/**
 * The portal's origins, for the endpoints the portal itself calls (request-book):
 * `https://<platform.portal.domain>`, and the portal's own Pages project,
 * `https://<portal.host.project>.pages.dev` plus one-label previews of it. Nothing
 * else. A registry with no portal block accepts nothing.
 *
 * @returns {{ resolve(origin: string): { ok: true, origin: string } | { ok: false, reason: string } }}
 */
export function createPortalResolver(registry) {
  const portal = registry.platform?.portal;
  const exact = new Set();
  let project = null;
  if (portal && typeof portal.domain === 'string' && HOSTNAME_RE.test(portal.domain)) exact.add(`https://${portal.domain}`);
  if (portal?.host?.provider === 'cloudflare-pages' && PAGES_PROJECT_RE.test(portal.host.project ?? '')) project = portal.host.project;

  return {
    resolve(origin) {
      if (typeof origin !== 'string' || !origin) return { ok: false, reason: 'no origin' };
      if (exact.has(origin)) return { ok: true, origin };
      const m = PAGES_ORIGIN_RE.exec(origin);
      if (project && m && m[1] === project) return { ok: true, origin };
      return { ok: false, reason: exact.size ? 'not the portal' : 'registry has no portal' };
    },
  };
}
